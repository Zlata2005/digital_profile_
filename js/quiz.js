// -------------------- РЕЗУЛЬТАТЫ ПРОФЕССИЙ ДЛЯ КВИЗА --------------------
...
let currentQuestionIndex = 0;          // Индекс текущего вопроса (0-9)
let scores = { dev: 0, analytics: 0, security: 0, management: 0 }; // Баллы по профессиям
let userAnswers = [];                  // Массив выбранных индексов (null если не отвечен)
let selectedOptionIndex = null;        // Индекс выбранного варианта на текущем экране
const totalQuestions = quizQuestions.length; // Всего вопросов = 10

// Инициализируем массив ответов значениями null
...
function renderQuestion() {
    ...
    questionText.textContent = q.question;          // Устанавливаем текст вопроса
    optionsContainer.innerHTML = '';               // Очищаем контейнер вариантов

    const savedAnswer = userAnswers[currentQuestionIndex];
    selectedOptionIndex = savedAnswer;             // Восстанавливаем предыдущий выбор (если был)
    ...
    q.options.forEach((opt, idx) => {
        ...
        if (idx === savedAnswer) btn.classList.add('selected'); // Если ранее был выбран – подсвечиваем
    ...
    updateNavigationButtons(); // Обновляем состояние кнопок Назад/Далее
}
...
function updateNavigationButtons() {
    prevBtn.disabled = (currentQuestionIndex === 0);   // На первом вопросе кнопка "Назад" отключена
    nextBtn.disabled = (selectedOptionIndex === null); // "Далее" активна только если выбран вариант
    nextBtn.textContent = (currentQuestionIndex === totalQuestions - 1) ? 'Завершить' : 'Далее →';
}

// -------------------- ПЕРЕСЧЁТ БАЛЛОВ НА ОСНОВЕ ВСЕХ ОТВЕТОВ --------------------
...
function handleNext() {
    if (selectedOptionIndex === null) return; // Ничего не выбрано – ничего не делаем

    // Сохраняем ответ текущего вопроса
    userAnswers[currentQuestionIndex] = selectedOptionIndex;
    recalculateScores(); // Пересчитываем баллы

    if (currentQuestionIndex < totalQuestions - 1) {
        currentQuestionIndex++;
        renderQuestion(); // Переход к следующему вопросу
    } else {
        showResult(); // Если это был последний вопрос – показываем результат
    }
}
...
function handlePrev() {
    if (currentQuestionIndex > 0) {
        ...
        currentQuestionIndex--;
        renderQuestion(); // Переход к предыдущему вопросу
    }
}

// -------------------- ПОКАЗ РЕЗУЛЬТАТА --------------------
function showResult() {
    recalculateScores(); // Финальный пересчёт

    // Определяем профессию с максимальным количеством баллов
    let maxCategory = 'dev';
    for (let key in scores) {
        if (scores[key] > scores[maxCategory]) {
            maxCategory = key;
        }
    }
    ...
    // Скрываем экран вопросов, показываем результат
...
function resetQuiz() {
    ...
    // Переключаем экраны: показываем стартовый, скрываем квиз и результат
...
// -------------------- ЗАПУСК КВИЗА --------------------
function startQuiz() {
    // Скрываем стартовый экран, показываем экран вопросов
    startScreen.classList.add('hidden');
    quizScreen.classList.remove('hidden');
    renderQuestion(); // Рисуем первый вопрос
}

// -------------------- ПРИВЯЗКА ОБРАБОТЧИКОВ СОБЫТИЙ (гарантированно после загрузки DOM) --------------------
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (restartBtn) {
        ...
        console.error('Кнопка restart-btn не найдена');
    }
    // Кнопки навигации
