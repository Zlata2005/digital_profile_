...
// Например, profession-detail.html?id=1
const urlParams = new URLSearchParams(window.location.search);
const professionId = parseInt(urlParams.get('id'));

// -------------------- НАХОДИМ ПРОФЕССИЮ В МАССИВЕ (из data.js) --------------------
...
if (!profession) {
    container.innerHTML = `
        <div style="text-align: center; padding: 40px;">
            ...
            <p>Возможно, был выбран неверный идентификатор.</p>
            <a href="professions.html" class="btn">Вернуться к списку</a>
        </div>
    `;
} else {
    // -------------------- ОПРЕДЕЛЯЕМ КЛАСС ДЛЯ БЕЙДЖА ВОСТРЕБОВАННОСТИ --------------------
    ...
    // -------------------- ФОРМИРУЕМ ПОЛНУЮ СТРАНИЦУ С ДАННЫМИ --------------------
    container.innerHTML = `
        ...
        <ul style="list-style: none; padding: 0;">
            ${companyNames || '<li>Нет данных</li>'}
        </ul>
        
        <h3><i class="fas fa-graduation-cap"></i> Где учиться</h3>
        <ul style="list-style: none; padding: 0;">
            ${eduNames || '<li>Нет данных</li>'}
        </ul>
